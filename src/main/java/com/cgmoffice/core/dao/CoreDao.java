...
public class CoreDao extends SqlSessionDaoSupport {
	...
	/**
	 * 리스트 조회 처리 SQL mapping 을 페이징처리로 실행한다.
	 *
	 * @param queryId - 리스트 조회 처리 SQL mapping 쿼리 ID
	 * @param pageBounds - 페이징처리를 위한 기본셋팅값
	 * @return 결과 PageList 객체 - SQL mapping 파일에서 지정한  resultType/resultMap 에 의한 결과 객체(보통 VO 또는 Map)의 PageList
	 */
	@SuppressWarnings("unchecked")
	public <E> PageList<E> selectListPage(String queryId, PageBounds pageBounds) {
		return (PageList<E>) getSqlSession().selectList(queryId, null, pageBounds);
	}

	@SuppressWarnings("unchecked")
	public <E> PageList<E> selectListPage(String queryId, PageConfig pageConfig) {
		PageList<E> pageList = (PageList<E>) getSqlSession().selectList(queryId, null, new PageBounds(pageConfig));
		int startCurNum = getStartCurNum(pageList);
		for(E e : pageList) {
			if(e instanceof Map) {
				Map map = (Map) e;
				map.put("rowno", startCurNum);
			} else {
				Class<?> dtoClass = e.getClass();
				AtomicInteger atmi = new AtomicInteger(startCurNum);
				Arrays.stream(dtoClass.getDeclaredFields())
				.filter(a -> a.getName().equals("rowno"))
				.findAny()
				.ifPresent(a -> {
					try {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <E> PageList<E> selectListPage(String queryId, Object parameterObject, PageBounds pageBounds) {
		return (PageList<E>) getSqlSession().selectList(queryId, parameterObject, pageBounds);
	}

	@SuppressWarnings("unchecked")
	public <E> PageList<E> selectListPage(String queryId, Object parameterObject, PageConfig pageConfig) {
		PageList<E> pageList = (PageList<E>) getSqlSession().selectList(queryId, parameterObject, new PageBounds(pageConfig));
		int startCurNum = getStartCurNum(pageList);
		for(E e : pageList) {
			if(e instanceof Map) {
				Map map = (Map) e;
	...
	@SuppressWarnings("unchecked")
	public <E> PageList<E> selectListPage(String queryId, Object parameterObject, int page) {
		return (PageList<E>) getSqlSession().selectList(queryId, parameterObject, new PageBounds(page));
	}
...
//	}
	...
	public int getStartCurNum(PageList pageList) {
