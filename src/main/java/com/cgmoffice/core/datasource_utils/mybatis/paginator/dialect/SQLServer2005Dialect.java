...
public class SQLServer2005Dialect extends Dialect{
    ...
    protected String getLimitString(String sql, String offsetName,int offset, String limitName, int limit) {
		...
		String loweredString = sql.toLowerCase(Locale.KOREA);
		...
		result.append(distinctStr);
		result.append("TOP 100 PERCENT ");
		result.append(" ROW_NUMBER() OVER (");
		result.append(orderby);
		result.append(") as __row_number__, ");
		result.append(pagingBuilder);
		result.append(") SELECT * FROM query WHERE __row_number__ > ? AND __row_number__ <= ?");
		result.append(" ORDER BY __row_number__");
